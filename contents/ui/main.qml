...
PlasmoidItem {
    ...
    function contrastColor(hex) { if (!hex || hex.length < 7) return Kirigami.Theme.textColor; var r = parseInt(hex.slice(1,3),16); var g = parseInt(hex.slice(3,5),16); var b = parseInt(hex.slice(5,7),16); var yiq = (r*299 + g*587 + b*114) / 1000; return yiq >= 170 ? '#000' : '#FFF' }
    ...
    function gameCenterUrl(away, home, start, gameId){
        ...
        var y = d.getFullYear(); var m = pad2(d.getMonth()+1); var da = pad2(d.getDate());
    ...
    function buildFromRawGames(games, errors){
        games=games||[];
        const now=new Date(); const startWin=new Date(now.getTime()-24*3600*1000); const endWin=new Date(now.getTime()+24*3600*1000);
        function inWindow(g){ const t=new Date(g.startTimeUTC||now); return t>=startWin && t<=endWin }
        let filtered=games.filter(g=>inWindow(g));
        if(!showAll && favoriteTeams.length){ filtered=filtered.filter(function(g){ const h=g.homeTeam&&g.homeTeam.abbrev; const a=g.awayTeam&&g.awayTeam.abbrev; return favoriteTeams.indexOf(h)>=0 || favoriteTeams.indexOf(a)>=0 }) }
        let byId={}; for(let i=0;i<filtered.length;i++){ const g=filtered[i]; byId[g.id]=g }
        let uniq=Object.keys(byId).map(k=>byId[k]);
    ...
    Connections {
        ...
        function onFavoritesChanged() { root.favoriteTeams = (Plasmoid.configuration.favorites||"").split(/\s*,\s*/).filter(s=>s.length>0); refresh() }
        function onShowAllChanged() { refresh() }
        function onTodayOnlyChanged() { refresh() }
    }

    // Unified style tokens for team chips (white text/border)
    readonly property color chipText: 'white'
    readonly property color chipBorder: 'white'
    readonly property int   chipBorderWidth: 1

    Component { id: statusBadge
        Rectangle { property string gameStatus: 'UPCOMING'; property string suffix: ''
            radius: 6; color: statusColor(gameStatus)
            height: stText.implicitHeight + 2; width: stText.implicitWidth + 8; opacity: 0.9
            Text { id: stText; anchors.centerIn: parent; text: (statusText(parent.gameStatus) + parent.suffix); color: 'white'; font.pixelSize: 11; font.bold: true }
        }
    }
    ...
    Component { id: teamColumn
        Column {
            ...
            property color bg: teamColor(code)
            Rectangle { radius: 5; color: bg; border.color: chipBorder; border.width: chipBorderWidth
                height: nameText.implicitHeight + 2; width: nameText.implicitWidth + 8
                Text { id: nameText; anchors.centerIn: parent; text: code; color: chipText; font.pixelSize: 11; font.bold: true; scale: 0.95 }
            }
            Text { text: String(score); font.pixelSize: 14; font.bold: true; color: Kirigami.Theme.textColor
    ...
    Component { id: teamRowInline
        Row {
            ...
            Rectangle { radius: 5; color: teamColor(awayCode); border.color: chipBorder; border.width: chipBorderWidth
                height: aText.implicitHeight + 2; width: aText.implicitWidth + 8
                Text { id: aText; anchors.centerIn: parent; text: awayCode; color: chipText; font.pixelSize: 11; font.bold: true }
            }
            ...
            Rectangle { radius: 5; color: teamColor(homeCode); border.color: chipBorder; border.width: chipBorderWidth
                height: hText.implicitHeight + 2; width: hText.implicitWidth + 8
                Text { id: hText; anchors.centerIn: parent; text: homeCode; color: chipText; font.pixelSize: 11; font.bold: true }
            }
        }
    }
    ...
    compactRepresentation: Item {
        ...
        Row { id: row; anchors.verticalCenter: parent.verticalCenter; spacing: 8; visible: todayGames.count > 0
            Repeater { model: todayGames
                delegate: Row {
                    visible: index < compactMaxGames; spacing: 10
                    Loader {
